...
const NotificationsManager = () => {
  ...
  const userNotifications = notifications.filter(
    (n) => n.notification.notificationType === 'user'
  );
      ...
      {showNotifications && (
        <div ref={notificationRef} className="absolute md:right-10 right-0 left-0 md:left-auto top-20 bg-white rounded-md shadow-lg border-2">
          <div className="flex justify-around border-b p-2">
            ...
            <button
              className={`relative px-4 py-2 ${
                activeTab === 'user' ? 'font-bold border-b-2 border-blue-500' : ''
              }`}
              onClick={() => setActiveTab('user')}
            >
              <p className="hidden md:block text-lg text-black font-bold m-2 mx-4">Utilisateurs</p>
              <FaUser className="block md:hidden text-green-500 text-xl" />
              {unreadUserNotifications.length > 0 && (
                <span className="absolute top-0 right-1 md:top-4 md:right-2 h-5 w-5 bg-red-500 text-white text-xs font-bold flex items-center justify-center rounded-full">
                  {unreadUserNotifications.length}
                </span>
          ...
          <div className="p-4 bg-[#f4f4f4] rounded-b-md max-h-96 overflow-y-scroll">
            {activeTab === 'order' && (
              <Notifications
