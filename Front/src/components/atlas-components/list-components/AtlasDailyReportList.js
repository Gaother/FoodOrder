...
import AtlasDashBoardRecentMoves from '../dashboard-components/AtlasDashBoardRecentMoves';
import api from '../../../api/api'; // Assurez-vous d'avoir le bon chemin vers votre instance d'API

const AtlasDailyReportList = () => {
    ...
    if (matchingReport) {
      return (
        <div className={`bg-gray-200 py-2  text-center`}>
          ...
          <AtlasDashBoardRecentMoves calendar={true} dailyReportId={matchingReport._id} />
        </div>
      );
    } else {
...
export default AtlasDailyReportList;
